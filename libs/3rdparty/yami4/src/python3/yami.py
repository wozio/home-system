...
def _append_long(buffer, i):
    ...
    buffer.write(struct.pack("<q", i.value))
...
def _append_string_content(buffer, s):
    ...
    buffer.write(bytes(s, "utf-8"))
    length = len(s)
    length_rounded = _round_to_4(length)
    buffer.write((length_rounded - length) * b"\x00")
        ...
        if e != None:
            ...
            if t == bool:
                ...
                if t == bool:
                    ...
                    packed_array = bytearray(bytes_needed * b"\x00")
                    ...
                    _append_int(buffer, length)
                    _append_binary_content(buffer, packed_array)
...
class Parameters(object):
    ...
    def __extract_string_content(self, buf, offset, length):
        ...
        value = str(buf[offset:(offset + length)], "utf-8")
    ...
    def __do_deserialize(self, buf, offset):
        ...
        for i in range(num_of_entries):
            ...
            if type_code == 1: # bool
                ...
                self.__setitem__(name, bytes(value))
                ...
                for j in range(length):
                    byte_position = j // 8
                    bit_position = j % 8
                    value.append((packed_array[byte_position] &
                                  (1 << bit_position)) != 0)
                self.__setitem__(name, value)
        ...
        for i in range(len(self.__entries)):
            ...
            if e == None:
                ...
                if t == bool:
                    ...
                    buf.write(prefix + "long: " + str(value.value))
...
def _pointer(result):
    ...
    if yami4py.yami4_is_success(result):
        ...
        yami4py.yami4_destroy_result(result)
        raise YAMIError(str(error, "utf8"))
