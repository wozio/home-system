...
// both client and server parts expect the high priority message to be
    ...
    std::size_t total_byte_count)
{
    if (hint == &hint_for_short_message_progress)
    {
        ...
        {
            // make sure that the short message is in fact pushed first
            assert(long_message_pushed == false);
    ...
    std::size_t num_of_body_buffers)
{
    ...
    if (std::string(message_type, message_type_length) == "short")
    {
        ...
        assert(long_message_received == false);
...
int main()
{
    ...
    // prepare the short (high-priority) message header and body
