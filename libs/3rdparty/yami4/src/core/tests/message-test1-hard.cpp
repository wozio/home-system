...
// this is a simple messaging test with explicit hard close on the client side
// apart from the channel closing method, it is identical to message-test1
...
int main()
{
    ...
    // explicit hard close of the client connection
    // (this action is immediate)

    res = client_agent.hard_close(cd);
    assert(res == yami::core::ok);
